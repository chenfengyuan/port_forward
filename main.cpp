...
std::string get_request_path(std::string &in){
...
std::string get_302_location_path(boost::asio::streambuf & buf){
...
//            std::cout << path << std::endl;
    ...
    void add_response_content(std::vector<char> buf, std::size_t len){
        ...
        if(location_path.length() > 0){
            if(request.length() > 0){
                redirect_trace.set(location_path, request);
            }
//            std::cout << location_path << std::endl;
        ...
        if(result.length() > 0){
            if(request.length() > 0){
                ...
                auto request_tmp = redirect_trace.get(path);
